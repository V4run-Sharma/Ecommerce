<template>
    <div>
      ...
      <div v-if="order && order.length > 0">
        <h2>Cart ID: {{ order[0].cartId }}</h2> <!-- Assuming all items have the same cartId -->
        <h3>ITEMS:</h3>
        <ul>
  ...
  export default {
    ...
    setup() {
      ...
      const fetchOrderDetails = async () => {
        ...
        try {
          const response = await fetch(`http://10.20.3.79:8092/order/getOrderDetails?orderId=${orderId}`);
          if (!response.ok) {
